...
if [ -d "$CRANE_DIR" ] && [ -d "$CRANE_DIR/.git" ]; then
  ...
  # Find the remote that actually hosts xellDart/Crane. A pre-existing clone
  # may have 'origin' pointing to an upstream fork (e.g. lucasjinreal/Crane),
  # in which case fetching CRANE_BRANCH from 'origin' would fail.
  CRANE_REMOTE=""
  if git remote get-url origin >/dev/null 2>&1 \
     && git remote get-url origin 2>/dev/null | grep -q 'xellDart/Crane'; then
    CRANE_REMOTE="origin"
  else
    CRANE_REMOTE=$(git remote -v | awk '/xellDart\/Crane/ && /\(fetch\)$/ {print $1; exit}')
    if [ -z "$CRANE_REMOTE" ]; then
      info "Crane: no remote points to xellDart/Crane.git — adding 'nebuia' remote"
      git remote add nebuia "$CRANE_REPO_URL" || warn "Could not add 'nebuia' remote"
      CRANE_REMOTE="nebuia"
    fi
    info "Crane: using remote '$CRANE_REMOTE' for branch $CRANE_BRANCH"
  fi

  CURRENT_BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
  STASHED=false

  # Auto-stash any uncommitted changes (tracked + untracked) so checkout can
  # proceed. The stash is preserved — restore later with: git stash pop
  if ! git diff-index --quiet HEAD -- 2>/dev/null \
     || [ -n "$(git ls-files --others --exclude-standard 2>/dev/null)" ]; then
    STASH_MSG="nebuia-embs installer auto-stash $(date +%Y-%m-%d_%H:%M:%S) from $CURRENT_BRANCH"
    info "Crane has local changes on '$CURRENT_BRANCH' — stashing as: $STASH_MSG"
    if git stash push --include-untracked -m "$STASH_MSG"; then
      STASHED=true
    else
      warn "Could not stash Crane changes — skipping update to avoid clobbering them."
      cd "$PROJECT_DIR"
      ok "Crane directory: $CRANE_DIR"
      echo ""
      true  # don't exit the script, just skip the update
    fi
  fi

  if [ "$STASHED" = true ] || git diff-index --quiet HEAD -- 2>/dev/null; then
    if git fetch "$CRANE_REMOTE" "$CRANE_BRANCH"; then
      if git rev-parse --verify "$CRANE_BRANCH" >/dev/null 2>&1; then
        git checkout "$CRANE_BRANCH" || warn "Could not checkout $CRANE_BRANCH"
        git merge --ff-only "$CRANE_REMOTE/$CRANE_BRANCH" \
          || warn "Crane: cannot fast-forward (diverged?). Continuing with local version."
      else
        git checkout -b "$CRANE_BRANCH" "$CRANE_REMOTE/$CRANE_BRANCH" \
          || warn "Could not create local branch $CRANE_BRANCH"
      fi
    else
      warn "Crane: fetch from '$CRANE_REMOTE' failed. Continuing with local version."
    fi

    if [ "$STASHED" = true ]; then
      warn "Your previous changes are saved in 'git stash'. To restore them:"
      warn "  cd $CRANE_DIR && git checkout $CURRENT_BRANCH && git stash pop"
    fi
  fi
  cd "$PROJECT_DIR"
else
...
if [ "$HAS_FLASH_ATTN" = true ]; then
  FEATURES+=("flash-attn")
  # Persistent cache outside /tmp so flash-attn artifacts survive reboots
  FA_CACHE_DIR="${HOME}/.cache/nebuia/flash_attn"

...
pub async fn get_document(pool: &PgPool, document_id: &str) -> anyhow::Result<Option<Document>> {
    let doc = sqlx::query_as::<_, Document>("SELECT status FROM documents WHERE id = $1")
        .bind(document_id)
        .fetch_optional(pool)
        .await?;
    Ok(doc)
...
pub async fn get_document_with_pages(
    ...
    let pages = sqlx::query_as::<_, Page>(
        "SELECT page_number, image_path FROM pages WHERE document_id = $1 ORDER BY page_number",
    )
    .bind(document_id)
    .fetch_all(pool)
    .await?;
